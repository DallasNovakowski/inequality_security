...
spending_pretest_splitplot <- spending_summary_data %>%
  ...
  # geom_label_repel(aes(label = paste(round(mean,2)), group =as.factor(cost)), fill="white", position = position_dodge(width = 0.7))+
    geom_label(
    #   # label.size = .1,
    #   # label.padding = unit(0.25, "lines"),
    #   # size=2.5,
      aes(label = paste(round(mean,2)), group =as.factor(cost),vjust=-1.25), fill="white", position = position_dodge(width = 0.7))+
...
effectsize::eta_squared(pretest_anova, alternative="two.sided", generalized = FALSE)

effectsize::omega_squared(pretest_anova, alternative="two.sided")

pretest_anova$cohen_f_p <- round(pretest_cohensf$Cohens_f_partial,3)

pretest_anova$ci95_lo <- round(pretest_cohensf$CI_low,3)

pretest_anova$ci95_hi <- round(pretest_cohensf$CI_high,3)

pretest_anova
```

```{r aov-anova}
aov_anova <- aov(security_spending ~ prob*cost + Error(participant_code/prob*cost), data=data_long)
...
```{r lme4-anova}
pretest_lme4 <- lme4::lmer(security_spending ~ prob*cost + (1 | participant_code), data = data_long, REML=TRUE)

lme4_anova <- round(anova(pretest_lme4, ddf = "Kenward-Roger"),3)
...
```{r rstatix-anova}
rstat_pretest_anova <- rstatix::anova_test(dv = security_spending, within= c(prob,cost), 
  wid=participant_code, data = data_long)

rstatix::get_anova_table(rstat_pretest_anova)

cgwtools::resave(pretest_cohensf,pretest_anova,tidy_pretest_model,pretest_model,pretest_icc,cor_50_60,cor_60_75,cor_50_75, file = here::here("output", "pretest_output.RData"))
```
...
ez_pretest <- aov_ez(id='participant_code',
                            dv='security_spending',
                            data=data_long,
                            within = c('prob','cost'),include_aov = TRUE)
ez_pretest

ez_pretest_peta <- effectsize::eta_squared(ez_pretest, alternative="two.sided",verbose = FALSE,generalized = FALSE)


afex_plot(ez_pretest,  ~ prob,  ~ cost, error = "within")
```
...
em1 <- emmeans(ez_pretest, 'prob')
em2 <- emmeans(ez_pretest, 'cost')
...
```{r ezANOVA}
pretest_ezANOVA <- ez::ezANOVA(data=data_long,dv = .(security_spending), 
                   wid = .(participant_code),
                   within = .(prob,cost),
                   type = 3, detailed = TRUE)

pretest_ezANOVA

pretest_ezANOVA <- pretest_ezANOVA$ANOVA


ezANOVA_lims <- conf.limits.ncf(F.value = .585, conf.level = 0.90, df.1 <- 2, df.2 <- 325)
ezANOVA_Lower.lim <- ezANOVA_lims$Lower.Limit/(ezANOVA_lims$Lower.Limit + df.1 + df.2 + 1)
ezANOVA_Upper.lim <- ezANOVA_lims$Upper.Limit/(ezANOVA_lims$Upper.Limit + df.1 + df.2 + 1)
ezANOVA_Lower.lim
ezANOVA_Upper.lim

var_names <- paste("ez_limits_", pretest_ezANOVA[1:4,1], sep = "")
var_names[x]
for (x in 1:nrow(pretest_ezANOVA)) {
print(conf.limits.ncf(F.value = pretest_ezANOVA[x,6], conf.level = 0.90, df.1 <- pretest_ezANOVA[x,2], df.2 <- pretest_ezANOVA[x,4]))
}

# ci.pvaf(F.value=0.53900775, df.1=, df.2=130, N=66, conf.level=.90)

# ezANOVA_cohensf <- effectsize::cohens_f(pretest_ezANOVA, alternative="two.sided",verbose = FALSE)
...
#The df for the residuals is the product of the numbers 
#of levels of all factors times the number of replicates minus 1. 


ezANOVA_lims_int <- conf.limits.ncf(F.value = 374.96205548, conf.level = 0.90, df.1 <- 1, df.2 <- 65)
ezANOVA_lim_lower_int <- ezANOVA_lims_int$Lower.Limit/(ezANOVA_lims_int$Lower.Limit + df.1 + df.2 + 1)
ezANOVA_lim_upper_int <- ezANOVA_lims_int$Upper.Limit/(ezANOVA_lims_int$Upper.Limit + df.1 + df.2 + 1)

ezANOVA_lims_prob <- conf.limits.ncf(F.value = .539, conf.level = 0.90, df.1 <- 2, df.2 <- 130)
ezANOVA_lim_lower_prob <- ezANOVA_lims_prob$Lower.Limit/(ezANOVA_lims_prob$Lower.Limit + df.1 + df.2 + 1)
ezANOVA_lim_upper_prob <- ezANOVA_lims_prob$Upper.Limit/(ezANOVA_lims_prob$Upper.Limit + df.1 + df.2 + 1)


ezANOVA_lims_cost <- conf.limits.ncf(F.value = 0.0343992, conf.level = 0.90, df.1 <- 1, df.2 <- 65)
ezANOVA_lim_lower_cost <- ezANOVA_lims_cost$Lower.Limit/(ezANOVA_lims_cost$Lower.Limit + df.1 + df.2 + 1)
ezANOVA_lim_upper_cost <- ezANOVA_lims_cost$Upper.Limit/(ezANOVA_lims_cost$Upper.Limit + df.1 + df.2 + 1)


ezANOVA_lims_cost_prob <- conf.limits.ncf(F.value = 0.41822008, conf.level = 0.90, df.1 <- 2, df.2 <- 130)
ezANOVA_lim_lower_cost_prob <- ezANOVA_lims_cost_prob$Lower.Limit/(ezANOVA_lims_cost_prob$Lower.Limit + df.1 + df.2 + 1)
ezANOVA_lim_upper_cost_prob <- ezANOVA_lims_cost_prob$Upper.Limit/(ezANOVA_lims_cost_prob$Upper.Limit + df.1 + df.2 + 1)

```


```{r aov-anova, eval = F}
# needs to be conducted on wide data 

model <- aov(security_spending ~ prob*cost + Error(factor(participant_code)), data = data_long)
summary(model)


#analyze model
car_aov_pretest <- car::Anova(model, type = 3)
