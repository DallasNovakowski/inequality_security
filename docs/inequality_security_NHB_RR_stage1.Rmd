...
# csl                 : "C:\Users\dalla\Zotero\styles\human-nature.csl"
...
# template: bookdown-article.template.tex
...
load(here::here("output", "manuscript_objects.RData"))
...
Currently, the most rigorous investigation of how inequality impacts distrust in populations is found in a simulation study by Decourson and Nettle @decourson2021. They found that in unequal environments, worse-off individuals will tend to be in poverty, and will try to exploit cooperative groups (as suggested by risk-sensitivity theory), leading others in a population to eventually avoid cooperation altogether. @decourson2021's theory is explicitly focused on poverty (i.e. absolute deprivation). However, in many instances, poverty and inequality do not co-occur. For instance, some workplaces and neighborhoods have little to no absolute deprivation, but they can still exhibit substantial stratification; in such a circumstance, @decourson2021's compositional model of inequality would not predict any reduced trust. We argue that considerations of others' *relative deprivation* can help to extend this inequality-trust theory to other important contexts.
...
The first experiment will manipulate high versus low economic inequality in a hypothetical context, to test whether **H1)** `r tolower(hypotheses[1])` (Table \@ref(tab:design-table)), versus **H1~0~)** no significant effect of inequality. Participants will be asked to imagine that they are paired with another participant, where they will each be assigned to different roles (Blue or Yellow), and endowed with money. In the equal condition, participants will be told that their partner received the same amount of funds as themselves (\$2). In the unequal condition, participants will be told that their partner instead received only \$1. This procedure has a notable strength by controlling the participant's income across levels of inequality (as the partner's money is the only income that changes). Results will be analyzed using an independent-samples t-test.

#### A priori power analysis (Study 1a)

Using mean and standard deviation values of security expenditure (*M*=`r round(base_mean,2)`, *SD*=`r round(base_sd,2)`) from the pretest, an a priori power analysis has been conducted using r package pwr (REF). Given the unknown size of the hypothesized effect, this study will target an effect size of cohen's d = .2, a small effect according to statistical convention [@cohen1988]. To achieve 95% power with one tail, studies must have a minimum of n = `r round(power_ttest_security$n,0)` participants per cell, or a study-wide total of n=`r format(round(power_ttest_security$n*2,0),big.mark = ",", scientific = FALSE)`.

### Study 1b: Inequality with real payoffs

The use of hypothetical scenarios in Study 1a may limit the reliability of any results. For instance, compared to hypothetical rewards, @xu2016 found that participants take fewer risks when incentivized by real payoffs and experiencing negative feedback [@hertwig2001]. Study 1b will serve to replicate study 1a, with the sole difference being that **H1)** (Table \@ref(tab:design-table); versus **H1~0~)**) is being tested when participants believe they are playing with a real partner, and whether their decision has real stakes. Results will be analyzed using an independent-samples t-test.

### Study 1c: Stake Size

The same inequality experiment will be conducted as in Study 1b, with the exception of manipulating the stake size of participants' (hypothetical decision). This study will serve to further replicate the positive effect of inequality on security consumption (**H1)**;Table \@ref(tab:design-table); versus **H1~0~)**), while testing the generalizability of the effect. Results will be analyzed using an independent-samples t-test.

### Study 1d: Meritocracy

Groups will always be characterized by inequality through some type of currency, whether wealth, income, or status. Indeed, a popular idea is that inequality propels individuals towards productive behaviours (REF). In this line, the presence of inequality is less of an issue compared to the distributional fairness in how the inequality was produced. The next two experiments will test whether **H2)** `r tolower(hypotheses[2])` (Table \@ref(tab:design-table); versus **H2~0~)** no significant inequality\*merit interaction). Study 1d again varies inequality (yes/no) as the previous studies, but also manipulates whether one's income is allocated *randomly or based on task performance*.

For this random vs. merit manipulation, all participants will complete a slider task, where they have one minute to correctly place 24 sliders at the midpoint of various lines that appear on their screens. Participants will then either be told that their incomes were allocated randomly, or because they correctly placed more sliders than their partner. The slider task has been used in several studies to examine the effects of effort in distributional preferences, and has demonstrated that individuals that win their income through performance are less likely to support redistribution of effort-driven income allocations [@cassar2019]. Participants will be excluded from analyses if they complete fewer than 3 sliders, which would both indicate low engagement in the study and likely undermine the credibility of the manipulation. Results will be analyzed using a 2x2 between-subjects ANOVA.
...
As described in the literature review to Study 1d, fairness appears to have its roots in avoiding partiality. Since Study 1d directly compares meritocratic versus random allocation of resources, it does not answer the more fundamental question: do people consume more security goods when inequality appears to be more discriminatory and unfair? Study 1e will keep a constant presence of inequality, but manipulate whether the partner is a member of an ingroup or outgroup, and whether the incomes are distributed randomly, or by a separate *ingroup individual*. Study 1e will use a 2x2 between subjects ANOVA to test whether **H3)** `r tolower(hypotheses[3])` (Table \@ref(tab:design-table); versus **H3~0~)** no significant outgroup/\*agentic-distribution interaction).
...
As a proof of concept, the proposed analyses have been conducted on individuals' victimization experiences, which, like security behaviours, is operationalized as a summed variable of multiple binary items, resulting in a similar distribution to security behaviours (see Figures \@ref(fig:icvs-security-hist) and \@ref(fig:icvs-victim-hist)). In this case, `r model_nonconvergence` of 100 analyzed dataframes failed to converge (preventing calculation of standard errors). As such, mixed ordinal regression without frequency weighting was conducted, and had no convergence issues. In this model, gini has a nonsignificant rtelationship with personal experiences of victimization, `r tidy_extract(tidy_ci,"gini_2004_6_cent")`. By contrast, all of the individual-level variables had significant relationships with likelihood to be report victimization, e.g., age: `r tidy_extract(tidy_ci,"age_cent")`. A coefficient of `r tidy_ci[6,"estimate"]` corresponds to a `r tidy_ci[6,"estimate"]` decrease in the natural logged odds of observing a 3 versus a 2, 1 or 0, per unit change in the age variable (which corresponds to a five-year change in real age). Similarly, a one-unit change in the age variable decreases the natural logged odds of observing a 2 versus 1 or 0 by `r tidy_ci[6,"estimate"]`. See Figure \@ref(fig:icvs-victim-dotwhisk) and Table \@ref(tab:s2-victim-table) for more results.
