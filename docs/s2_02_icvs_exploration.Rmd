...
The following describes two survey sweeps of the International Crime Victimization Survey conducted across 2004-6. At first, this data had `r sweep5_nrow` respondents and `r sweep5_countries` countries. After a exclusion process maximizing the number of countries and observations, there were

31 countries with 85,085 participants, with a minimum of 790 participants per country.

Analyzing archival data, the so-called "garden of forking paths [\@gelman2014] allows a researcher to pick and choose those data and analyses that best suit their preferred story [\@simmons2011]. As such, transparently showcasing the different analysis paths is an important measure to help reduce the encroachment of model selection bias.

As mentioned in the manuscript, certain variable inclusions (e.g., perceived police effectiveness) requires dropping certain countries of Japan, Peru, and Iceland (`mod_joined1`: k = `r sum(summary(as.factor(mod_joined1$country))>0)`, and n = `r format(nrow(mod_joined1), big.mark = ",", scientific = FALSE)`.
...
To give a clearer picture of the data being excluded, data exploration will first be presented for the more complete dataset, then presented with the smaller subset.
...
```{r hist-info, warning=F, message=FALSE, fig.show='hide'}
