...
The following and planned analyses will be carried out with `r sum(summary(mod_joined1$country)>0)` countries, and `r format(nrow(mod_joined1), big.mark = ",", scientific = FALSE)` participants.
...
# Square root method
mod_joined1$sqrt_victimization <- sqrt(mod_joined1$num_victim_5yr)
  # normality_stats(mod_joined1$sqrt_victimization)

# Cube root method
mod_joined1$cubrt_victimization <- '^'(mod_joined1$num_victim_5yr,1/3) # cube root transformation
  #normality_stats(mod_joined1$cubrt_victimization)

# rankit method
mod_joined1$rankit_victimization <- RNOmni::RankNorm(mod_joined1$num_victim_5yr) # rank-based inverse normal transformation (approximately normalize most distribtutional shapes, and which effectively minimizes type I errors and maximizes statistical power; Bishara & Hittner, 2012; Puth et al., 2014
  #normality_stats(mod_joined1$rankit_victimization)

#ggplot(mod_joined1, aes(x = rankit_victimization))  +  geom_histogram(binwidth= 1, alpha=.7, fill="#40B0A6", colour='grey') + geom_density(adjust=6, aes(y=..count..)) + theme_minimal()

# Ordinal method
mod_joined1$ordinal_victimization <- NA
mod_joined1$ordinal_victimization <- ifelse(between(mod_joined1$num_victim_5yr,0,1), 1, NA)
mod_joined1$ordinal_victimization[mod_joined1$num_victim_5yr == 0] <- 0 
mod_joined1$ordinal_victimization[mod_joined1$num_victim_5yr > 1] <- 2

# Log method
mod_joined1$log_victimization <- log10(mod_joined1$num_victim_5yr+1)
  #normality_stats(mod_joined1$log_victimization)
...
# Square root method
mod_joined1$sqrt_security <- sqrt(mod_joined1$total_security) # square root transformation seems to provide something closer, so just use that?

  #hist_plot(mod_joined1, sqrt_security, "Histogram of Total Security (Square root)")
  #normality_stats(mod_joined1$sqrt_security)


# Ordinal security
mod_joined1$ordinal_security <- NA

mod_joined1$ordinal_security <- ifelse(between(mod_joined1$total_security,0,1), 1, NA)
mod_joined1$ordinal_security[mod_joined1$total_security == 0] <- 0 
mod_joined1$ordinal_security[mod_joined1$total_security > 1] <- 2
  # hist_plot(mod_joined1, ordinal_security, "Histogram of Ordinal Security")
  # normality_stats(mod_joined1$ordinal_security)

# log+1 method
mod_joined1$log_security <- log10(mod_joined1$total_security+1)
  # hist_plot(mod_joined1, log_security, "Histogram of Logged Security")
  # normality_stats(mod_joined1$log_security)


# inverse method
mod_joined1$inv_security <- 1/(mod_joined1$total_security+1)
  # hist_plot(mod_joined1, inv_security, "Histogram of inverted Security")

# Box Cox Method
security_boxcox<- geoR::boxcoxfit(mod_joined1$total_security, lambda=0, lambda2 = 1) 
  # security_boxcox <- MASS::boxcox(mod_joined1$total_security)
```
...
```{r security summary}
summary(mod_joined1$total_security)

normality_stats(mod_joined1$total_security)
```

This summing procedure gives us a max score of `r max(mod_joined1$total_security)` on security consumption, with substantial positive skew
...
m_ord_null <- clmm(ordered(num_victim_5yr_winz) ~ 1 + (1|country), data = mod_joined1)

# country not found
# m_ord <- clmm(ordered(num_victim_5yr) ~ age_cent + gdppc_2004_6, random  = country, data = mod_joined1)
          
m_ord <- clmm(ordered(num_victim_5yr_winz) ~ gini_2004_6_cent + gdppc_2004_6_scale + age_cent + employed + male + police_effective + income_quartile + (1|country), data = mod_joined1)

summary(m_ord)

anova(m_ord_null, m_ord)

mw_ord <- clmm(ordered(num_victim_5yr_winz) ~ gini_2004_6_cent + gdppc_2004_6_scale + age_cent + employed + male + police_effective + income_quartile + (1|country), data = mod_joined1, weights = individual_weight)
...
m_ord2 <- clmm2(ordered(num_victim_5yr) ~ gini_2004_6_cent + gdppc_2004_6_scale + age_cent + employed + male + police_effective + income_quartile, random = country, data = mod_joined1, Hess = TRUE)
...
swiid_responders <-  swiid %>%
  map(. %>% filter(.,country %in% names(summary(mod_joined1$country)[0!= summary(mod_joined1$country)]) )) %>%
...
pwt_responders <-  pwt100_gdppc %>%
  filter(country%in% names(summary(mod_joined1$country)[0!= summary(mod_joined1$country)]) )
```
...
cr_vals <- cr_setup(mod_joined1$num_victim_5yr_winz)

cr_data <- mod_joined1[cr_vals$subs, ]
...
m1_null <- lme4::lmer(num_victim_5yr ~ (1 | country), data = mod_joined1, REML=FALSE)

# Estimate model
m1_sample <- lme4::lmer(num_victim_5yr ~ gini_2004_6_cent + gdppc_2004_6_scale + 
                     age_cent + employed + male + police_effective + income_quartile + (1 | country), data = mod_joined1, REML=FALSE)
...
m1_weight <- lme4::lmer(num_victim_5yr ~ gini_2004_6_cent + gdppc_2004_6_scale + age_cent + employed +   male + (1 | country),data = mod_joined1, REML=FALSE, weights = individual_weight)
```
...
mod_joined1[,c("num_victim_5yr", "num_victim_5yr_winz")] %>%
  summary()
...
plot(fitdist(mod_joined1$total_security,"pois"))
plot(fitdist(mod_joined1$num_victim_5yr,"pois"))


# ggplot(data = mod_joined1,
#        mapping = aes(sample = total_security)) + 
#   stat_qq(distribution = stats::qpois,
#           dparams = list(lambda = mean(mod_joined1$total_security))) + 
...
mean(mod_joined1$total_security)
# print("Security variance is")
var(mod_joined1$total_security)

# print("Victimization mean is")
mean(mod_joined1$num_victim_5yr)
# print("Victimization variance is")
var(mod_joined1$num_victim_5yr)
```
...
m_pois <- glmer(num_victim_5yr ~ gdppc_2004_6_scale + gini_2004_6_cent + age_cent + employed + male + police_effective + income_quartile + (1 | country), family = poisson, data = mod_joined1)
```
...
plotResiduals(simulationOutput, form = mod_joined1$gini_2004_6_cent)
```
...
# vcdExtra::zero.test(mod_joined1$total_security)
```
...
# m_zip <- glmmTMB::glmmTMB(num_victim_5yr ~ gdppc_2004_6_scale  + av_gini + age_cent + employed + male + police_effective + income_quartile + (1 | country),zi = ~ gdppc_2004_6_scale  + av_gini + age_cent + employed + male, data = mod_joined1, family = poisson)
...
# m_bino <- glmer.nb(num_victim_5yr ~ gdppc_2004_6_scale + gini_2004_6_cent + age_cent + employed + male + police_effective + income_quartile + (1 | country),data = mod_joined1)
# 
...
# summary(!is.na(mod_joined1[["num_victim_5yr"]]))
# which(is.na(mod_joined1[["num_victim_5yr"]]))
# 
# mod_joined1$num_victim_5yr_winz_1 <- mod_joined1$num_victim_5yr_winz + 1
# 
# norm <- MASS::fitdistr(mod_joined1$num_victim_5yr_winz_1, "normal")
# 
# # qqp(mod_joined1$num_victim_5yr, "norm", main = "Normal distribution")
# 
# qqp(mod_joined1$num_victim_5yr_winz_1, "norm", main = "Normal distribution")
# 
# 
# qqp(sqrt(mod_joined1$num_victim_5yr_winz), "norm", main = "Normal distribution, square root")
# # qqp(sqrt(mod_joined1$num_victim_5yr_winz), "norm", main = "winzorized and square root")
# 
# qqp(mod_joined1$num_victim_5yr_winz_1, "lnorm", main = "Logged normal distribution")
# # qqp(mod_joined1$num_victim_5yr_winz + 1, "lnorm", main = "Winzorized and Logged")
# 
# # negative binomial - fails to optimize with +1 transformation
# # nbinom <- MASS::fitdistr(mod_joined1$num_victim_5yr_1, "Negative Binomial")
# # qqp(mod_joined1$num_victim_5yr_1, "nbinom", main = "Negative binomial distribution", size = nbinom$estimate[[1]], mu = nbinom$estimate[[2]])
# 
# 
# #poisson distribution
# poisson <- MASS::fitdistr(mod_joined1$num_victim_5yr_winz_1, "Poisson")
# qqp(mod_joined1$num_victim_5yr_winz_1, "pois", main = "Poisson distribution", lambda = poisson$estimate)
# 
# 
...
# qqp(mod_joined1$num_victim_5yr_winz+1, "pois", main = "Winzorized Poisson distribution", lambda = poisson$estimate)
...
iv_long <- gather(mod_joined1, measure, victimized, cartheft_5yrs:assault_5yrs, factor_key=TRUE)
